...
> **위협 모델 & 전제 (Threat Model).** Red는 시스템을 장악하지 못하고 **최소 접근 권한**만 가진다고 가정합니다 — AI가 받는 `blue_observed`의 외형(값·시간·순서·메타데이터)만 조작할 수 있고, 암호는 깨지 못합니다. **이유:** 완벽한 접근을 가정하면 공격이 "그냥 전원을 끈다" 같은 시시한 해로 붕괴되어 방어·시뮬레이션이 무의미해지기 때문입니다. 따라서 복호화 없이 관측 계층을 교란하는 현실적 공격만 다루고, 직접 복호화·시스템 장악은 범위 밖(out_of_scope) 카탈로그로 남깁니다. (근거: `docs/encrypted_channel_attack_ai.md`)
