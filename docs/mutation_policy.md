...
| `external_observe.telemetry_channels` | 송출/수신 telemetry projection. Red가 읽고 기억할 수 있는 intel 자원 | 금지 |
...
`loud_demo`는 현재 코드의 `timestamp_delta_s = -400`, 큰 ACK delay/jitter 같은 시연용 큰 변조를 설명하기 위한 profile이다. 현재 MVP 기본값은 `aggressive`이며, `stealth`는 낮은 폭의 경계 탐색이나 adaptive retry에 사용한다. `TELEMETRY_FDI`는 더 이상 telemetry 값을 직접 변조하지 않고, read-only telemetry tx/rx memory를 근거로 ACK/command/timing 혼동을 만든다.
...
Telemetry field policies are retained for legacy hard-case scorer/defense fixtures and future scenario packs.
The implemented `TELEMETRY_FDI` mutation path uses ACK sequence, ACK delay, latency, jitter, and optional command decoy fields instead.
...
## 7. Mutation Approval Reviewer

Red 내부에 `Mutation Approval Reviewer`를 둔다. 이 reviewer는 **reviewer-only**다. 공격을 선택하거나, 변조값을 새로 만들거나, `blue_observed`를 직접 수정하거나, payload를 만드는 모델이 아니다. Attack Selector와 deterministic Mutation Policy가 만든 후보 mutation을 보고 `approve`, `clamp`, `reject`, `explain`만 수행한다. 구현 위치는 `src/dah_flawless/mutation_review/`이고, 외부 LLM 설정은 `configs/mutation_approval_reviewer.json`에서 관리한다.
...
외부 LLM 출력과 deterministic policy가 충돌하면 deterministic policy가 항상 이긴다. 외부 연결 실패, invalid JSON, schema 검증 실패가 발생하면 오프라인 heuristic reviewer가 같은 인터페이스로 즉시 대체된다. 최종 state 변경은 `Mutation Engine`만 수행한다.
