...
> 최종 갱신: 2026-07-04 (**용어 경계 정리 + raw_world pipeline 이식 + Situation Tagger/Attack Selector + scorer window 실사용**)

---

## 0. 한 줄 현황

**현재 개발 방향은 raw_world → Feature Extractor → State Adapter → scorer_truth(state["world"]) + blue_observed → Situation Tagger → Red/Blue Agents → Scorer/Admin 흐름이다.** 창빈 base 위에 world generator/extractor/adapter, encrypted-channel situation tags, Attack Selector, tactic scoring, raw-world 대시보드 입력, scorer detection/recovery window 실사용까지 이식/수정했다. `state["world"]`는 raw_world가 아니라 scorer-only truth라는 점을 문서/로그/테스트에 반영했다. 모든 변경은 아직 **로컬 미커밋**. **다음: 보고서에 raw_world pipeline과 Red AI 구조를 반영 → 테스트 결과 갱신 → 커밋/push 조율 → 최종 PDF/ZIP.**
...
| `docs/llm_alignment_guide.md` | 새 세션/다른 LLM용 용어 정렬 안내 스크립트 |
| `docs/raw_world_schema.md` | 현실 원천 신호(raw_world) schema와 pipeline |
| `docs/DAH2026_integrated_plan.md` | 창빈 통합 플래닝 |
| `docs/implementation_plan.md` | 창빈 구현 계획 |
| `docs/schema_design.md` · `field_formats.md` | scorer_truth(`state["world"]`)·blue_observed 데이터 구조·필드 형식 |
| `docs/situation_tags.md` · `attack_mapping.md` | 태그·공격 매핑 |
| `docs/world_observed_model.md` | raw_world/scorer_truth/blue_observed 개념 |
...
2. **진실 분리:** raw_world는 현실 원천 신호, scorer_truth는 `state["world"]` 호환 키, blue_observed는 AI 입력이다. Red·Blue 둘 다 scorer_truth를 안 봄. Blue는 redacted state만.
...
### 4.4 창빈 코드 자체 개선 — ✅ 진행
- ✅ **detection_window / recovery_window 실사용** — `score_round(..., threat_history=..., recovery_history=...)`로 `DETECTION_WINDOW=2`, `RECOVERY_WINDOW=2`를 scorer evidence와 판정에 반영.
- ✅ **용어 정리** — 로그 필드를 `raw_world_source_hash`, `raw_world_feature_scores`, `truth_model`, `truth_storage_key`로 정리. `world_source_hash` 같은 애매한 이름 제거.
- ✅ **Streamlit raw world 입력** — 사이드바에서 raw-world JSON/JSONL sample을 받아 `build_state_from_raw_world()`로 초기 상태 생성.
...
```powershell
git status --short
$env:PYTHONDONTWRITEBYTECODE='1'; $env:PYTHONPATH='src'; python -m unittest discover -s tests
...
기대값:
- `python -m unittest discover -s tests` → **38 tests OK**
...
6. 정리 후 다시 `python -m unittest discover -s tests`를 돌린다.
...
- 2026-07-04 — **용어 경계/LLM 안내/Raw-world pipeline 정리.** `docs/llm_alignment_guide.md`와 `scripts/print_llm_alignment_guide.py` 추가. `raw_world`, `scorer_truth(state["world"])`, `blue_observed` 경계를 README/HANDOFF/schema 문서에 반영. 로그 필드를 `raw_world_source_hash`, `raw_world_feature_scores`, `truth_model`, `truth_storage_key`로 정리. `DETECTION_WINDOW`/`RECOVERY_WINDOW` scorer 실사용, Streamlit raw-world sample 입력 추가. `unittest discover` **38개 통과**.
