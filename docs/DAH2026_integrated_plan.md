...
> 용어 업데이트(2026-07-04): 이 문서의 예전 `world` 표현은 현재 구현 기준으로 `scorer_truth(state["world"])`에 해당한다. 현실 원천 신호는 `raw_world`, Blue 입력은 `blue_observed`로 구분한다. 최신 기준은 `docs/llm_alignment_guide.md`와 `docs/world_observed_model.md`를 우선한다.
...
- **Must(코어):** 공격 3종 E2E · raw_world/scorer_truth/observed 분리 · 불변식 탐지 · 공격별 Detect/Contain/Recover 로그 · scorer 판정 · seed 재현 · 8항목 보고서 매핑 · **보고서 필수 도해/그림 세트(§10, 산출물은 repo 밖에서 관리)**.
- **Should(가점):** 공진화 그래프(static vs coevolution) · availability 곡선 · 방어 큐 타임라인 · 신뢰예산/피로스 정량화 · degraded 시나리오.
- **Nice(여유):** Streamlit 라이브 대시보드 · 데모 영상 · LLM 보고서 · 공격 4종 이상 · mid_incident 다수.

> 핵심: **보고서에 박히는 정적 그림(PNG)은 점수 본체라 Must**지만, 현재 코드 repo에는 생성된 그림/PDF/ZIP을 보관하지 않는다. Streamlit 라이브 대시보드·데모 영상은 부가 보조재다.
...
| Red 조작 데이터 | `world{battery:20, motor:FAULT}` → `observed{battery:82, motor:OK}`. `82`는 큰 변조를 보여주는 `loud_demo` profile 예시 |
...
```text
1. observed state ─► Red Agent : observed만 조작 (scorer_truth는 안 받음)
2. environment    : scorer_truth는 보존, observed만 변형해 state 갱신
3. redacted state ─► Blue 4-Agent : Threat → Mission → Planner → Report
4. scorer         : scorer_truth와 observed를 비교해 승패 판정 (scorer만 scorer_truth 접근)
5. scorer ──► Red : "성공/실패" 피드백만 전달 (Blue 내부는 안 알려줌)
* Orchestrator가 1~5를 매 라운드 돌리고 로그를 남긴다.
```

한 줄 핵심: **`scorer_truth(state["world"])`는 environment와 scorer에만 남고, Red도 Blue도 직접 받지 않는다.** Red는 `observed`만 변형하고, scorer_truth와 벌어졌는지는 scorer가 판정한다. 이 구조가 "Red/Blue 모두 진실을 직접 못 본다"는 차별점을 한눈에 보여준다(강점). 보고서에는 이 흐름을 박스 다이어그램(그림)으로 그린다.
...
> Red는 거짓을 만들 때 scorer_truth가 필요 없다. "배터리 +60 부풀려"처럼 `observed`만 조작하고, scorer_truth와 벌어졌는지는 scorer가 판정한다. → Red 입력에서 scorer_truth 완전 제거(모순 해소, 게임 공정).
...
- [ ] repo 구성: `README` · `src/` · `tests/` · `requirements.txt` 또는 `Dockerfile` · `docs/` · `configs/`
- [ ] 주장→증거 추적표 포함
...
{ "round": 0, "situation_tags": ["RECON_APPROACH","GNSS_PRIMARY"],
  ...
  "observed": {"battery_percent":82,"motor":"OK"},             // 조작 대상(loud_demo profile 예시)
...
예선은 **PDF 보고서 심사**다. 점수를 받는 건 보고서에 박힌 **정적 그림/표(PNG)**이고, 라이브 대시보드·영상은 부가 보조재다. 다만 현재 repo에는 생성된 그림/PDF/ZIP을 고정 보관하지 않는다.

### 10.1 보고서 정적 그림/표 (Must — 점수 본체)

현재 브랜치에서는 정적 그림 생성기를 repo에 두지 않는다. 보고서 작성 단계에서는 `round_logs.jsonl`/`summary.json` 또는 Streamlit 화면을 근거로 별도 작업공간에서 seed 고정 그림을 만든다.
...
> 협력 다이어그램·공격 흐름도처럼 데이터가 아니라 개념을 그리는 도해는 Figma 등 그림 도구로 만들고, World vs Observed·점수판·곡선처럼 로그 기반 그림은 별도 산출물로 생성한다.
...
### 10.3 라이브 대시보드 & 데모 영상 (Nice)

- **Streamlit 4탭(Nice):** Overview·Timeline·State Diff·Charts. **Scorer Truth vs Observed 탭은 `Scorer/Admin View`로 명시 분리** — Blue가 scorer_truth를 본다는 오해 차단.
- **데모 영상(Nice):** "CLI 실행 → 로그 생성 → 대시보드 확인" 화면녹화. 예선 필수 아님, 코어 끝난 뒤.

---

## 11. MVP 합격선 (이게 충족되면 "예선 제출 가능")

1. 공격 3종이 각각 조작 데이터를 생성하고 `scorer_truth`/`observed`를 실제로 벌린다.
2. **Blue 입력에는 scorer_truth 키 `state["world"]`가 절대 포함되지 않는다.** (테스트로 강제)
...
| 약점 | 해결 |
|---|---|
| 진실 누출(Blue가 scorer_truth 봄) | redacted state, scorer만 진실, 테스트로 강제(§11-2) |
...
| P3a | 7/4~7/6 | 협력 다이어그램 + 에이전트 decision_log + 운용 서사 + 별도 보고서 그림 산출 | Must |
...
**추천 실행 순서:** 8항목 목차 확정 → 공격 3종 운용 시나리오 → Detect/Contain/Recover 표 → 1종 E2E 로그 → 3종 로그 → 협력 다이어그램 + 별도 보고서 그림 세트 → seed+README+Docker → 공진화/availability 그래프 → (라이브 대시보드/영상) → (LLM).
...
| Blue는 scorer_truth 미접근 | `test_redact.py` 통과 | `environment/simulator.py` | 협력 다이어그램 | C | ⑥ |
...
7. `test_redact.py`(scorer_truth 키 제거 강제) + seed 재현 테스트.
8. 공격 3종 + 공격별 Detect/Contain/Recover 로그.
9. 협력 다이어그램 + 에이전트 decision_log + 운용 서사 초안.
10. 별도 보고서 그림 산출 — `round_logs.jsonl` 또는 Streamlit 화면을 근거로 World vs Observed·점수판 PNG(seed 고정)를 만든다. repo에는 생성물을 저장하지 않는다.
