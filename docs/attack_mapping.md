...
Blue는 world를 보지 않고 blue_observed 내부의 불변식 위반만 본다.
Scorer만 world와 blue_observed를 비교해 성공 여부를 판정한다.
```
...
| 예시 | 실제 배터리 20%, 모터 고장인데 observed는 배터리 82%, 모터 OK |
...
```text
TELEMETRY_FDI는 실제 UAV 상태를 바꾸지 않고, Blue 관제 AI가 수신하는 텔레메트리 값을 오염시킨다. Blue는 world를 알지 못하지만, 배터리 보고값과 소모율, 모터 상태의 물리 정합성 위반을 통해 조작 가능성을 탐지한다.
```
...
| 탐지 태그 | `SEQUENCE_REGRESSION`, `TIMESTAMP_SKEW`, `REPLAY_SUSPECTED` |
| Blue 탐지 단서 | sequence 역행, timestamp skew, 이전 메시지 패턴 반복 |
| 방어 | 명령 보류, 마지막 정상 명령 유지, 재검증 요청 |

보고서 문장:

```text
TIME_DESYNC_REPLAY는 C2 메시지의 sequence number와 timestamp를 교란하여 과거 명령이 최신 명령처럼 처리되도록 유도한다. Blue는 메시지 내용 전체를 복호화하지 못하더라도 sequence, timestamp, 인증 메타데이터의 이상을 통해 replay 가능성을 탐지한다.
```
...
```text
attack_success
= 공격이 노린 target domain에서 world와 blue_observed가 불일치하고,
