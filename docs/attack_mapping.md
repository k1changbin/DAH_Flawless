...
|---|---|
| 공격명 | `TELEMETRY_FDI` |
| 목적 | telemetry tx/rx 기록을 read-only intelligence로 보고 Blue의 명령/ACK 해석을 흔듦 |
| 조작 대상 | `blue_observed.c2_message.ack.sequence_number`, `blue_observed.comms.ack_delay_ms`, `blue_observed.comms.latency_ms`, `blue_observed.comms.packet_interval_jitter_ms`, optional `blue_observed.c2_message.command` |
| world 기준 | `world.uav.*`로부터 만들어진 `telemetry_channels.asset_tx_mirror`, `ground_rx_view` context |
| 예시 | 실제 배터리 20%, 모터 고장 context를 읽은 뒤 telemetry 값은 유지하고 ACK sequence를 어긋나게 하며 `CONTINUE_MISSION` decoy를 섞음 |
| 탐지 태그 | `TELEMETRY_INTERNAL_TX_DISAGREE`, `TELEMETRY_TX_RX_DISAGREE`, `TELEMETRY_RX_COMMAND_INCONSISTENT`, `TELEMETRY_FRESHNESS_RISK`, `ACK_TIMING_ANOMALY` |
| Blue 탐지 단서 | internal vs tx, tx vs rx, rx vs command, freshness/link quality를 분리한 telemetry channel checks |
| 방어 | telemetry 신뢰도 하향, command hold, ACK/timing 재검증, 마지막 정상 상태 fallback |

보고서 문장:

```text
TELEMETRY_FDI는 scorer_truth 기준 UAV 상태와 telemetry channel projection을 직접 바꾸지 않는다. Red는 송출/수신 telemetry 기록을 읽기 전용으로 기억한 뒤, ACK sequence, ACK delay, link timing, command decoy를 통해 Blue의 telemetry 신뢰 판단과 명령 해석을 혼동시킨다.
```
...
|---|---|---|---|
| `TELEMETRY_FDI` | telemetry-memory 기반 ACK/timing/command ambiguity | telemetry 신뢰도 하향 + command/ACK 재검증 | 마지막 정상 상태 fallback, 복귀 판단 |
...
```text
attack_success
= 공격이 노린 target domain에서 scorer evidence가 목표 축의 임계값을 넘은 경우.
  TELEMETRY_FDI는 직접 battery_delta보다 telemetry_command_confusion,
  stale_state_acceptance, wrong_safety_decision을 우선한다.
...
```text
TELEMETRY_FDI:
telemetry_channels.asset_tx_mirror.battery_percent = 20
blue_observed.telemetry.battery_percent = 20
blue_observed.c2_message.ack.sequence_number = 1019
blue_observed.comms.ack_delay_ms = 950
score.evidence.goal_score.evidence.telemetry_command_confusion >= 0.30
score.evidence.goal_score.evidence.stale_state_acceptance >= 0.35
score.evidence.goal_score.evidence.wrong_safety_decision >= 0.35
Blue가 ACK_TIMING_ANOMALY와 TELEMETRY_MEMORY_COMMAND_CONFUSION을 탐지하지 못하고 CONTINUE_MISSION을 신뢰
=> RED_BREACH
```
