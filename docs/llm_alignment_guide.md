...
7. Blue 학습 규칙에서 DETECTION_BOUNDARY_PROBE는 meta goal이다.
- 이 값 자체를 독립 effect로 과적합시키지 말고, mission-impact component 또는 실제 감지된 하위 EFFECT_*로 remap한다.
- C2 명령 복구는 가능한 경우 현재 internal_observe.c2_message를 trusted anchor로 사용한다.
...
DAH_Flawless/
  ...
  docs/
    ...
    schema_design.md
    field_formats.md
    blue_availability_recovery_model.md
    blue_defense_effect_contracts.md
    situation_tags.md
    attack_mapping.md
...
Blue readiness gate is applied on top of the alternating cadence. If recent
Blue defense success is below the configured threshold, planned Red update
episodes are converted into Blue-only update episodes: Red policy/goal weights
stay frozen, while Blue detection and defense policy continues to learn.
...
현재 구현에서는 `TrainingScheduler`가 이 cadence를 실행한다. Red policy state는 공격 weight, adaptive stealth set, telemetry probe delta를 저장한다. Blue policy state는 rule-based baseline에서 쓰는 `domain_trust`, `detection_sensitivity`, `escalation_threshold`, `feedback_counts`와 effect별 `effect_sensitivity`, `effect_threshold`, `effect_feedback_counts`, `effect_mission_impact_ema`를 저장한다. Blue update block에서는 Red policy를 고정하고 Blue policy만 업데이트하며, Red update block에서는 Blue policy를 고정하고 Red weight/probe만 업데이트한다. Fixed evaluation block에서는 둘 다 고정한다. `HoldoutEvaluator`는 이 최종 policy state를 복사해 scenario pack 전체에서 업데이트 없이 평가하며, MVP coverage용 scripted attack은 꺼서 학습된 policy 선택을 평가한다. 단, holdout case 사이의 이전 로그는 selector context로 전달해 frozen policy가 한 공격으로 붕괴하지 않도록 attack diversity penalty만 적용한다.
...
Rolling Log Memory는 긴 round-mode run에서 Red planning context를 압축한다. 원본 JSONL audit log는 유지하지만, Red Goal Planner와 Attack Selector가 받는 `previous_logs`는 일정 라운드마다 `proxy_logs + recent_logs`로 초기화된다. proxy logs는 기존 경향성을 따르되 reward/mission-impact와 탐지/복구 여부에 작은 bounded variation을 넣어 장기 반복 고착을 줄인다.
...
| Goal Planner | 어떤 cyber-effect 목표를 노릴지 선택. 예: 오표적 선택, 명령 stale 처리, telemetry trust erosion | `attacks/goal_planner.py`. 현재 context와 이전 로그를 함께 보는 contextual UCB-style scoring + 최근 목표/domain 반복 감점 diversity guard |
| Attack Selector | tag, 후보 점수, Attack-Effect Contract 정합성, 최근 공격 반복 감점으로 공격 선택 | `attacks/selector.py`, `attacks/effect_contracts.py`, `red_agent.py` |
...
| Feedback Learner | scorer 결과로 domain/effect sensitivity/threshold 업데이트, effect별 mission-impact 보정 | `blue/feedback_learner.py` |
...
- Blue availability는 단순 체력이 아니라 임무 지속성 예산이다. 방어 action은 지연, 격리, 재검증, fallback 때문에 availability/trust_budget을 깎지만, 그 손상은 한 round-level combat episode 안에서만 유효하다. 라운드 시작 시 `round_episode_budget_reset_v1`이 시나리오 초기 예산으로 리셋한다. 상세 근거는 `docs/blue_availability_recovery_model.md`를 기준으로 한다.
- Blue containment는 완전 복구가 아니다. `containment_score`는 탐지 후 effect pressure를 얼마나 줄였는지 보는 중간 지표이고, `recovery_success`는 trusted state로 완전히 복원된 경우에만 true로 둔다. 상세 기준은 `docs/blue_defense_effect_contracts.md`를 기준으로 한다.
...
| `EFFECT_DETECTION_BOUNDARY_PROBE` | 낮은 amplitude의 단일 effect 징후와 detector feedback 확보 가능성. 학습 시 독립 effect가 아니라 실제 하위 effect로 remap되는 meta goal |
...
Attack-Effect Contract는 공격 후보와 기대 cyber-effect를 연결하는 안전한 시뮬레이션 계약이다. 실제 RF/API/payload 절차를 설명하지 않고, `supported_goal_ids`, `mutation_paths`, `expected_tags`, `expected_effect_tags`, `success_evidence_keys`만 정의한다. Attack Selector는 contract alignment를 후보 점수에 반영하고, Goal-aware Scorer는 unsupported attack-goal pair를 low-reward 실패로 clamp한다.

공격 다양성을 늘릴 때 contract를 완화하면 안 된다. 현재 구현은 contract-compatible tactic exploration과 repeat guard를 사용한다. 즉 정합성 없는 attack-goal pair는 막고, 정합성 있는 tactic 후보 안에서 반복 penalty와 exploration을 적용한다.

Mission-impact scoring은 `attack_success`나 `goal_success`를 대체하지 않는다. observe 오염이 임무 판단, telemetry safety, command freshness, availability pressure를 얼마나 흔들었는지 별도 점수로 남기고, contract-supported goal의 `goal_reward`에만 제한적으로 섞는다. 정합성 없는 attack-goal pair는 mission impact가 높아도 reward clamp를 우회할 수 없다.

Causal Consistency Monitor는 attack -> observe mutation -> situation/effect tag -> scorer evidence 체인이 유지되는지 검사한다. 이 값은 `causal_consistency` 라운드 로그와 summary의 `avg_causal_consistency`, `causal_warning_count`, `causal_failure_count`에 남는다. `attack_entropy`, `tactic_entropy`는 다양성 붕괴를 보는 지표다.
...
`score.goal_reward`는 Red Goal Planner/Feedback Learner가 목표별 가중치를 업데이트할 때 우선 사용하는 reward다. 현재 구현은 goal effect score를 기본으로 하고, contract-supported 목표에 한해 mission-impact score를 제한적으로 섞는다. 외부 LLM이 이 값을 직접 생성하면 안 되고, scorer가 계산한 값을 해석하거나 reviewer로만 다룬다.
...
| `score.goal_reward` | Goal Planner/Feedback Learner용 목표별 reward. contract-supported 목표는 mission-impact 보정 포함 |
| `score.evidence.goal_score` | goal_id별 evidence와 effect_score |
| `score.evidence.mission_impact` | 임무 판단/안전/명령 freshness/가용성 영향 evidence |
...
| `blue_policy_state` | Blue domain/effect sensitivity, threshold, trust, feedback counts, effect별 mission-impact EMA |
| `policy_update_review` | policy delta 후보, selected scale, rejection count, fallback reason |
| `feedback` | scorer 결과를 policy update에 넘기는 요약. mission-impact score 포함 |
| `log_memory_event` | round-mode rolling memory compaction 요약과 proxy log 수 |
...
| state adapter | 구현 |
| situation tagger | 구현 |
| goal planner + diversity guard | 구현 |
| attack selector diversity penalty | 구현 |
| attack-effect contract | 구현 |
| causal consistency monitor | 구현 |
...
| feedback learner | Red/Blue 기본 구현, Blue mission-impact feedback 구현 |
| LLM adapter | 구현 |
